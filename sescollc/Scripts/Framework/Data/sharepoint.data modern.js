...
class $SPData { // He's creating a class and defining class functions to be used on this class - 'static' keyword
  ...
  * @param {string} webUrl
  */
    static GetListFields(listName, fields, webUrl) { // using this as a class function to be used to get the fields from the lists - what he's calling in CreateRequest.js
        //example: $SPData(class).GetListFields(function call)(parameters here);
        // arguments object is used to get the arguments in the actual function call - returns as an array - might be able to use this to call in the list I had to create for 
        //Responsible department 

        let e = Function.validateParameters(arguments, [{ name: 'listName', type: String, mayBeNull: false, optional: false },
        { name: 'fields', type: Array, elementMayBeNull: false, elementType: String, mayBeNull: false, optional: false },
        { name: 'webUrl', type: String, mayBeNull: false, optional: true }], true);
        if (e) throw e;

        let args_length = arguments.length;
        let promise = new Promise(function (resolve, reject) {
            //I think he's getting the current context of the SP list here - if args length is !== 2 creating a new webUrl which is a property in Object 'e'
            ...
            for (let i = 0; i < fields.length; i++) {
                let oField = oList.get_fields().getByInternalNameOrTitle(fields[i]);
                ctx.load(oField);
                oFields[fields[i]] = oField;
            }
    ...
    static DeleteItem = function (listName, itemId, webUrl) { // anonymous function declaration - function with no name 
    ...
    //function to get all items from a list - might have to use this to get the items to populate the dropdown
