...
function GetData() {
    ...
    //This runs when a ticket is selected from the master ticket list under Administration
    ...
    let getCompleteDate = $SPData.GetListFields('SupportTickets', ['CompleteByDate']);
   
    
    


    Promise.all([getTicket, getAttachments, getComments, getClosureReason, getFields, getCompleteDate]).then(function (results) {
        ...
        // Add completeByDate when getting tickets - added field to SupportTickets list 
        let completeByDate = ticket.get_item('CompleteByDate') === null ? '' : ticket.get_item('CompleteByDate').format('yyyy-MM-dd');
       
        
        

        /** 
         *  This is where the fields are populated when a ticket is selected from the list
         **/

        // $select('BusinessFunction').bindToField(results[4]['BusinessFunction'], ticket.get_item('BusinessFunction'));
        $select('BusinessFunction').set_value(ticket.get_item('BusinessFunction') === null ? '' : ticket.get_item('BusinessFunction').split(';'));
        $select('ResponsibleDepartment').set_value(ticket.get_item('ResponsibleDepartment') === null ? '' : ticket.get_item('ResponsibleDepartment'));
        ...
        // added to set the value of the text box
        $select('CompleteByDate').set_value(completeByDate);
        ...
        // populateSubmitter(ticket.get_item('Submtter').get_lookupValue().split('membership|')[1].split(',')[0]);
        var submitter = ticket.get_item('Submtter').get_email() || ticket.get_item('Submtter').get_lookupValue().split('membership|')[1].split(',')[0];
        populateSubmitter(submitter);
        $select('CreatedBy').set_value(ticket.get_item("Author").get_lookupValue());
        $select('TeamID').set_value(ticket.get_item("Team").get_lookupValue());
        ...
        if (ticket.get_item('ResponsibleDepartment') !== null) {
            $select('ResponsibleDepartment').set_value(ticket.get_item('ResponsibleDepartment').split(';'));
        }
       ...
       // DisablePhases();
        ...
        //Formatting for comments html - I think I can add in the formatting here - maybe - probably either here or up above where commented - ~200 LOC
        html = '';
        ...
        for (let k = 0; k < comments.get_count(); k++) {
            ...
            /*if (comment.toString().charAt(comment.length-1) == 'e') {
                comment += '\n';
            }*/
            let badgeColor = (k % 2 === 0 ? '#000' : '#fc3');
            let fmt_comment = comment.get_item('_Comments');
            let split_cmt = fmt_comment.split('\n');
            let array_cmt = split_cmt.map(word => '<p>' + word + '</p>');
            let pgraph_cmt = array_cmt.join('');
           
            
            //This is where he is splitting the comment blocks for the image - looks like a conversation in a text message
            html += (k % 2 === 0 ? '<li>' : '<li class="timeline-inverted">') +
                '<div class="timeline-badge warning" style="background-color:' + badgeColor + ' !important"><i class="fa fa-comment"></i></div>' +
                '<div class="timeline-panel">' +
                '<div class="timeline-heading">' +
                '<small class="text-muted">' + comment.get_item('Author').get_email() + ' on ' + comment.get_item('Created').format('yyyy-MM-dd HH:mm') + '</small>' +
                '</div>' +
                '<div class="timeline-body">' +
                pgraph_cmt +  // Updated this to make sure comments were spaced correctly - pretty sure it's working
                '</div>' +
                '</div>' +
                '</li>';
        }
...
function EditRequest() { // <---This is the method that gets called on save button press


    // Form validation
    let controls_to_validate = ['TicketTitle', 'TicketType', 'TicketPriority', 'TicketDescription', 'TicketType', 'BusinessFunction', 'TicketSubType', 'TicketComments', 'ClosureReasons', 'CompleteByDate', 'ResponsibleDepartment'];
    ...
    request['CompleteByDate'] = $select('CompleteByDate').get_value() === '' ? null : $select('CompleteByDate').get_value(); // trying to get date value
    ...
    let getAdminsCaml = '<View>' +
        ...
        '</Values>' +
        '</In>' +
        '<Eq><FieldRef Name="Team" LookupId="FALSE" /><Value Type="Lookup">' + $select('TeamID').get_value() + '</Value></Eq>' +
        '</And>' +
        '</Where>' +
        '</Query>' +
        '</View>';
    ...
    let comments = {};
    let newComments = $select('TicketComments').get_value().trim(); // All this is doing is removing the whitespace - it's not formatting the string - Maybe by design?
    ...
    Promise.all([$SPData.UpdateItem('SupportTickets', request), $SPData.AddItem('Comments', comments), $SPData.GetListItems('Users', getAdminsCaml)]).then(function (values) {
        let administrators = values[2];
        let adminEmails = [];
        for (let i = 0; i < administrators.get_count(); i++) {
            adminEmails.push(administrators.itemAt(i).get_item('UserEmail'));
        }

        let edited_request = values[0];
        SendEmail(edited_request, adminEmails);
...
function SendEmail(edited_request, adminEmails) {
    ...
    var body = '<table style="width: 100%; color:#ffffff; font-family:\'Arial\'">' +
        ...
        // Complete by Date
        '<tr>' +
        '<td style="background-color:#EEEEEE; height:22px; color:#000000; vertical-align: middle; width: 200px; border-bottom:1px #EEEEEE solid;">Complete By Date:</td>' +
        '<td style="height: 22px; color:#000000; vertical-align: middle; border-bottom:1px #EEEEEE solid;">' + $select('CompleteByDate').get_text() + '</td>' +
        '</tr>' +
        // Enhacement Required
        '<tr>' +
    ...
    emailProperties.add_cc(adminEmails);
