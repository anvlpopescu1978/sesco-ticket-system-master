function pageLoad(sender, args) {
    ...
    /*
    jq('#superAdminMenu, #requestersMenu').on('hidden.bs.collapse', function (e) {
        let sideMenuHeight = jq('#side-menu').height();
        alert(sideMenuHeight);
        let pageWrapperHeight = jq('#page-wrapper').height();
        if (sideMenuHeight > pageWrapperHeight) {
            jq('#page-wrapper').height(sideMenuHeight + 'px');
        }
    });


    jq('#superAdminMenu, #requestersMenu').on('shown.bs.collapse', function (e) {
        let sideMenuHeight = jq('#side-menu').height();
        alert(sideMenuHeight);
        let pageWrapperHeight = jq('#page-wrapper').height();
        if (sideMenuHeight > pageWrapperHeight) {
    ...
    */
    ...
    $SPData.GetListItems('Users', '<View><Query><Where><Eq><FieldRef Name="UserAccount" /><Value Type="Integer"><UserID /></Value></Eq></Where></Query></View>')
        .then(function (items) {
            ...
            if (_spPageContextInfo.isSiteAdmin === true) {
                jq('#usersArea, #teamArea, #adminArea, #superAdminArea').show();
                // Run page content load with  site admin role
                if (typeof (pageContentLoad) === 'function') {
                    if (items.get_count() === 0) {
                        pageContentLoad(sender, args, 'Super Administrator', null);
                    }
                    else {
                        pageContentLoad(sender, args, 'Super Administrator', items.itemAt(0).get_item('Team'));
                    }
                }
                return;
            }
            ...
            let team = items.itemAt(0).get_item('Team');
    ...
    Shp.Lists.GetItems('Users', null, query, function (items) {
        ...
        switch (role) {
            ...
            case 'Super Administrator':
                jQuery('#usersArea, #manageProductsMenu, #customerMenu, #assignedToMeMenu, #ordersAdminMenu, #currenciesAndCountriesMenu, #currenciesAndCountriesMenu, #manageUsers, #savetoexternallib, #contractTemplates, #superAdminArea').show();
                break;
            case 'Administrator':
...
function ShowMenuItem(item) {
    ...
    var sideMenuHeight = jq('#side-menu').height();
    var pageWrapperHeight = jq('#page-wrapper').height();
    if (sideMenuHeight > pageWrapperHeight) {
        jq('#page-wrapper').height(sideMenuHeight + 'px');
    }
}
...
function Search() {
    ...
    NavigateToPage('/Pages/Common/Search.aspx?criteria=' + jq('#srch-term').val().trim());
}
